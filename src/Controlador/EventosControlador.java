...
public class EventosControlador {
    ...
    /**
     * Método para validar la entrada de letras, espacios y puntos.
     * @param evt Evento KeyEvent generado al presionar una tecla.
     */
    public void validarLetrasEspaciosPuntosNum(KeyEvent evt) {
        char car = evt.getKeyChar();
        if (!esCaracterLetra(car) && !esCaracterNumero(car) && car != BACKSPACE && car != SPACE && car != DOT) {
            mostrarMensajeError("Solo se permiten letras, espacios y puntos.");
            evt.consume();
        }
    }
    
    public void validarLetrasEspaciosPuntos(KeyEvent evt) {
        char car = evt.getKeyChar();
        if (!esCaracterLetra(car) && car != BACKSPACE && car != SPACE && car != DOT) {
            mostrarMensajeError("Solo se permiten letras, espacios y puntos.");
            evt.consume();
        }
    }
